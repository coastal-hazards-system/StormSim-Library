...
filename = 'Example_LCS_Output_Timeseries.h5'; % Define Output H5 Filename
...
vars_desc = {'Storm_ID','Storm_Length','Timestep','Storm_Duration',...
    'Sim_Year','SWL','Hm0','Damage_Depth','Tp','Wave Direction'}; % Dataset Names
vars_att = {'','Number of timesteps','yyyy-mm-dd HH:MM','days','','m','m','m','s','deg (referenced to grid)'};
order_indx = [1 2 4 9 10 5 6 -9 7 8]; % Col Index For Each LC, -9 -> Damaging Depths
...
for sp = 1:length(sp_list)
    ...
    for lc = 1:length(data)
        ...
        for j = 1:length(order_indx) % Loop through each group
            % Define Dataset
            dset_details.Location = sprintf(['/SP_%d/LC_%d/' vars_desc{j}], sp_list(sp), lc); % Location
            dset_details.Name = vars_desc{j}; % Name
            % Grab Dataset
            if order_indx(j)~=-9 % From Data
                dset = data(lc).LCNUM(:,order_indx(j));
