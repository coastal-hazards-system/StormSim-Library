   ...
   of execution selected by the User, as follows:
    1) 'Regular': when ExecMode = 'Regular', the output structure
      ...
      (RL); the POT sample (POT); the outputs of the MRL method (MRL_out);
       ...
       HC (HC_tbl) for the values in HC_tlb_x; the summarized HC (HC_plt_rsp_x)
...
INPUT ARGUMENTS:
  ...
  - staID: gauge station ID number; as a character vector. Example: '8770570'
...
OUTPUT ARGUMENTS:
  ...
  - HC_tbl_x: predefined vector of probabilities used to summarize the HC. The type is:
     > AEP when use_AEP = 1
     > AEF when use_AEP = 0
  - HC_plt_rsp_y: vector of response values used to summarize the HC, based
        on resolution of HC_plt_x
  - HC_tbl_rsp_y: vector of response values used to summarize the HC, based
        on resolution of HC_tbl_x
  ...
  - SST_output: structure array containing the output data of the StormSim-SST
     ...
     > MRL_output: output of Mean Residual Life (MRL) function; as a structure array with fields:
         Summary = summary of the threshold selection results, as a table array with format:
            ...
            AD_GOF_pvalue: statistic of the Anderson-Darling GoF Test
     ...
     > HC_plt_rsp_x: hazard values interpolated from the HC, that correspond
         to the responses in HC_plt_rsp_y; as a structure array with two fields: 'out' and 'MRL_Crit'. It may contain up
         ...
         out = numerical array with the following 5 rows
            ...
            row(04): values of 84% percentile or 3rd percentage of input "prc"
            row(05): values of 98% percentile or 4th percentage of input "prc"
     > HC_tbl_rsp_x: same as HC_plt_rsp_x, except the resolution of the
            values is based upon the HC_tbl_rsp_y variable
         MRL_Crit = character vector indicating the MRL threshold
            selection criterion used for the HC_plt_rsp_x.out record.
...
20230125-JAM:  removed all staID cell brackets for PROS.  Assume that staID is single valued.
20230223-LAA: Major changes made. More staID cell brackets removed to enable the plotting 
    functions to work properly. Redefined the rsp_x vectors to have a lower resolution, based upon 
    the HC_tbl_x and HC_plt_x vectors.

    Reimplemented the Anderson-Darling test in the "StormSim_MRL.m" according to the 
    methodology described in "Threshold Detection for the Generalized Pareto Distribution" (Langousis, 2016).
    Eliminated the "KernReg_LocalMean.m" function in the "StormSim_MRL.m" and replaced 
    the smoothing metholdology with a prominence measure. Eliminated the "Monotonic_adjustment.m" function
    and replaced the methodology by adding the "includenan" option to the quantile computations. 

***************  ALPHA  VERSION  **  FOR INTERNAL TESTING ONLY ************
%}
function [HC_plt_x,HC_plt_rsp_y,HC_tbl_x,HC_tbl_rsp_y,Removed_datasets,Check_datasets,SST_output] = StormSim_SST_Tool_R1_v20230223_FGM(input_data,flag_value,tLag,lambda,Nyrs,path_out,staID,yaxis_Label,yaxis_Limits,...
              prc,use_AEP,GPD_TH_criterion,SLC,apply_Tides,gprMdl,DataType,ExecMode,HC_plt_rsp_y,HC_tbl_rsp_y,apply_GPD_to_SS,apply_Parallel,tide_SD,U_a,U_r,uncert_treatment)
%% Introduction
%clc;
% disp(['***********************************************************' newline...
...
switch ExecMode
    case 'Regular'
        SST_output = struct('staID','','RL',double.empty,'POT',double.empty,'MRL_output',double.empty,...
            'HC_plt',double.empty,'HC_tbl',double.empty,'HC_plt_rsp_x',double.empty,'HC_tbl_rsp_x',double.empty,'HC_emp',double.empty,'Warning','','ME',cell(1)); % Pre-allocate output structure array
    ...
    case 'Fast'
        SST_output = struct('staID','','HC_plt',double.empty,'HC_tbl',double.empty,'HC_plt_rsp_x',double.empty,'HC_tbl_rsp_x',double.empty,'Warning','','ME',cell(1)); % Pre-allocate output structure array
        ...
        if GPD_TH_criterion==0 % Evaluate all MRL thresholds
            for i=sz %dataset loop
                ...
                try
                    ...
                    [HC_emp,HC_plt,HC_plt_x2,HC_tbl,HC_plt_rsp_x,HC_tbl_rsp_x,MRL_output] = StormSim_SST_Fit(POT_samp(:,2),Nyrs(i),HC_plt_x,HC_tbl_x,HC_plt_rsp_y,HC_tbl_rsp_y,prc,use_AEP,GPD_TH_criterion,apply_Tides,procData(i).dat2,SLC,gprMdl(i).mdl,staID,yaxis_Label{i},path_out,yaxis_Limits(i,:),apply_GPD_to_SS,tide_SD(i),U_a,U_r,uncert_treatment);
                    ...
                    switch ExecMode
                        case 'Regular'
                            ...
                            SST_output(j).HC_plt_rsp_x=HC_plt_rsp_x;
                            SST_output(j).HC_tbl_rsp_x=HC_tbl_rsp_x;
                            SST_output(j).HC_emp = HC_emp;

                            % Plot results
                            StormSim_SST_Plot(HC_plt,HC_emp,MRL_output,prc,staID,yaxis_Label{i},path_out,yaxis_Limits(i,:),use_AEP,GPD_TH_criterion,a,HC_plt_x2)
        ...
        elseif id_PCT && GPD_TH_criterion~=0 && apply_Parallel % PCT available and evaluate only one MRL threshold
            for i=sz %dataset loop
                ...
                try
                    ...
                    [HC_emp,HC_plt,HC_plt_x2,HC_tbl,HC_plt_rsp_x,HC_tbl_rsp_x,MRL_output,str1] = StormSim_SST_Fit_SimplePar(POT_samp(:,2),Nyrs(i),HC_plt_x,HC_tbl_x,HC_plt_rsp_y,HC_tbl_rsp_y,prc,use_AEP,GPD_TH_criterion,apply_Tides,procData(i).dat2,SLC,gprMdl(i).mdl,apply_GPD_to_SS,tide_SD(i),U_a,U_r,uncert_treatment);
                    ...
                    switch ExecMode
                        case 'Regular'
                            ...
                            % Plot results
                            StormSim_SST_Plot_Simple(HC_emp,HC_plt,HC_plt_x2,MRL_output,prc,use_AEP,staID,yaxis_Label{i},path_out,yaxis_Limits(i,:),a,GPD_TH_criterion)
            ...
            for i=sz %dataset loop
                ...
                try
                    ...
                    % Execute StormSim-SST-Fit-Simple
                    [HC_emp,HC_plt,HC_plt_x2,HC_tbl,HC_plt_rsp_x,HC_tbl_rsp_x,MRL_output,str1] = StormSim_SST_Fit_Simple(POT_samp(:,2),Nyrs(i),HC_plt_x,HC_tbl_x,HC_plt_rsp_y,HC_tbl_rsp_y,prc,use_AEP,GPD_TH_criterion,apply_Tides,procData(i).dat2,SLC,gprMdl(i).mdl,apply_GPD_to_SS,tide_SD(i),U_a,U_r,uncert_treatment);
    ...
    case 'POT'
        if GPD_TH_criterion==0
            for i=sz %dataset loop
                ...
                %                 try
                % Execute StormSim-SST-Fit
                [HC_emp,HC_plt,HC_plt_x2,HC_tbl,HC_plt_rsp_x,HC_tbl_rsp_x,MRL_output] = StormSim_SST_Fit(procData(i).dat(:,2),Nyrs(i),HC_plt_x,HC_tbl_x,HC_plt_rsp_y,HC_tbl_rsp_y,prc,use_AEP,GPD_TH_criterion,apply_Tides,procData(i).dat2,SLC,gprMdl(i).mdl,staID,yaxis_Label{i},path_out,yaxis_Limits(i,:),apply_GPD_to_SS,tide_SD(i),U_a,U_r,uncert_treatment);
...
%                 disp(['*** Step 3: Performing SST for station ',staID])
                ...
                try
                    % Execute StormSim-SST-Fit-Simple
                    [HC_emp,HC_plt,HC_plt_x2,HC_tbl,HC_plt_rsp_x,HC_tbl_rsp_x,MRL_output,str1] = StormSim_SST_Fit_SimplePar(procData(i).dat(:,2),Nyrs(i),HC_plt_x,HC_tbl_x,HC_plt_rsp_y,HC_tbl_rsp_y,prc,use_AEP,GPD_TH_criterion,apply_Tides,procData(i).dat2,SLC,gprMdl(i).mdl,apply_GPD_to_SS,tide_SD(i),U_a,U_r,uncert_treatment);
...
%                 disp(['*** Step 3: Performing SST for station ',staID])
                ...
                try
                    % Execute StormSim-SST-Fit-Simple
                    [HC_emp,HC_plt,HC_plt_x2,HC_tbl,HC_plt_rsp_x,HC_tbl_rsp_x,MRL_output,str1] = StormSim_SST_Fit_Simple(procData(i).dat(:,2),Nyrs(i),HC_plt_x,HC_tbl_x,HC_plt_rsp_y,HC_tbl_rsp_y,prc,use_AEP,GPD_TH_criterion,apply_Tides,procData(i).dat2,SLC,gprMdl(i).mdl,apply_GPD_to_SS,tide_SD(i),U_a,U_r,uncert_treatment);
...
%% Save the output
save([path_out,'StormSim_SST_output.mat'],'SST_output','HC_tbl_x','HC_tbl_rsp_y','HC_plt_x','HC_plt_rsp_y','Removed_datasets','Check_datasets','-v7.3')
...
20220419-ERS: added AD GoF Test as additional criterion for threshold
    selection. Debugged.
20230223-LAA: Reimplemented the Anderson-Darling test in the "StormSim_MRL.m" according to the 
    methodology described in "Threshold Detection for the Generalized Pareto Distribution" (Langousis, 2016).
    Eliminated the "KernReg_LocalMean.m" function in the "StormSim_MRL.m" and replaced 
    the smoothing metholdology with a prominence measure. 
***************  ALPHA  VERSION  **  FOR INTERNAL TESTING ONLY ************
%}
...
% preallocate output
mrl=NaN(1,10);mrl = array2table(mrl,'VariableNames',{'Threshold','MeanExcess','Weight','WMSE','GPD_Shape','GPD_Scale','Events','Rate','AD_GOF_Test','AD_GOF_pvalue'});
...
if N>20
    ...
    mrl=NaN(N-10,10); %pre-allocation for speed
    % col(01): thresholds, u
    ...
    % col(09): Anderson-Darling Hypothesis Test Result (0-Do not Reject H0; 1-Reject H0)
    % col(10): Anderson-Darling Hypothesis Test p-value
    ...
    % Determine if to use analytical formula or limiting distribution to solve for the AD_GOF_pvalue
    if N > 120
        method = true;
    else
        method = false;
    ...
    for j = 1:N-20
        ...
        try
            ...
            dist = makedist('GeneralizedPareto','k',pd.k,'sigma',pd.sigma,'theta',pd.theta);
            mrl(j,5) = pd.k; %shape
            mrl(j,6) = pd.sigma;%scale
            [mrl(j,9),mrl(j,10)] = adtest(u,'Distribution',dist,'Alpha',0.05,'Asymptotic',method); %Decision for H0, p-value
        catch
        end
    end
    ...
    %Identify local minima  (Added " 'MaxNumExtrema',5 " - LAA 2022-10-05)
    TH_id = islocalmin(mrl(:,4),'MaxNumExtrema',5,'FlatSelection','first');
    ...
    if sum(TH_id)==0
        ...
        MRL_output.Selection.AD_GOF_pvalue(1) = mrl2(I,10);
        ...
        MRL_output.Selection.AD_GOF_pvalue(2) = mrl2(I,10);
    end

    %% Step 6: Threshold selection: Anderson-Darling GOF test criterion
    %Identify local minima
    TH_id = find(mrl(:,9) == 0); % Do Not Reject H0

    % If no min found, return empty arrays
    if ~isempty(TH_id)
        % Take lowest threshold with p-value > Alpha
        I = TH_id(1);
        mrl2=mrl(I,:); %take data of minima
        % Take results
        ...
        MRL_output.Selection.AD_GOF_pvalue(3) = mrl2(I,10);
    end

    %% Store MRL output
    MRL_output.Summary = array2table(mrl,'VariableNames',{'Threshold','MeanExcess','Weight','WMSE','GPD_Shape','GPD_Scale','Events','Rate','AD_GOF_Test','AD_GOF_pvalue'});
end
end
...
20230223-LAA: Redefined the rsp_x vectors to have a lower resolution, based upon 
    the HC_tbl_x and HC_plt_x vectors. Eliminated the "Monotonic_adjustment.m" function
    and replaced the methodology by adding the "includenan" option to the quantile computations. 
***************  ALPHA  VERSION  **  FOR INTERNAL TESTING ONLY ************
%}
function [HC_emp,HC_plt,HC_plt_x2,HC_tbl,HC_plt_rsp_x,HC_tbl_rsp_x,MRL_output] = StormSim_SST_Fit(POT_samp,Nyrs,HC_plt_x,HC_tbl_x,HC_plt_rsp_y,HC_tbl_rsp_y,prc,use_AEP,GPD_TH_criterion,apply_Tides,POT_samp2,SLC,gprMdl,staID,yaxis_Label,path_out,yaxis_Limits,apply_GPD_to_SS,tide_SD,U_a,U_r,uncert_treatment)
...
if (length(ecdf_y)>=20 && Nyrs>=20) || apply_GPD_to_SS %apply GPD
    ...
    for i=j %MRL GPD Threshold loop
        ...
        % preallocate
        HC_plt_rsp_x2 = NaN(size(Mean_p_CLs,1),length(HC_plt_rsp_y));
        ...
        for kk=1:size(Mean_p_CLs,1)
            ...
            % Interpolate
            HC_plt_rsp_x2(kk,:) = exp(interp1(dm1,dm2,HC_plt_rsp_y','linear','extrap'));
        ...
        HC_plt_rsp_x2(HC_plt_rsp_x2<1e-6)=NaN;
        HC_tbl_rsp_x2(HC_tbl_rsp_x2<1e-6)=NaN;
        ...
        % Store
        HC_plt_rsp_x(i).out = HC_plt_rsp_x2;
        HC_tbl_rsp_x(i).out = HC_tbl_rsp_x2;
        HC_plt_rsp_x(i).MRL_Crit = MRL_output.Selection.Criterion{i};
    ...
    for kk=1:size(Mean_p_CLs,1)
        ...
        % Interpolate
        HC_plt_rsp_x2(kk,:) = exp(interp1(dm1,dm2,HC_plt_rsp_y','nearest','extrap'));
    ...
    % Store
    HC_plt_rsp_x(1).out = HC_plt_rsp_x2;
    HC_tbl_rsp_x(1).out = HC_tbl_rsp_x2;
    HC_plt_rsp_x(1).MRL_Crit = 'None';
...
OUTPUT ARGUMENTS:
  ...
  - MRL_output: output of Mean Residual Life (MRL) function; as a structure array with fields:
     ...
     eMsg = Status message indicating when the MRL methodology was not able
        to objectively determine a theshold for the GPD. In this case, the
        GPD threshold is set to 0.99 times the minimum value of the bootstrap sample.
        Otherwise, eMsg will be empty.Monotonic
AUTHORS:
    Norberto C. Nadal-Caraballo, PhD (NCNC)
    Efrain Ramos-Santiago (ERS)
CONTRIBUTORS:
...
***************  ALPHA  VERSION  **  FOR INTERNAL TESTING ONLY ************
%}
function [HC_emp,HC_plt,HC_plt_x2,HC_tbl_y,HC_plt_rsp_x,HC_tbl_rsp_x,MRL_output,str1] = StormSim_SST_Fit_Simple(POT_samp,Nyrs,HC_plt_x,HC_tbl_x,HC_plt_rsp_y,HC_tbl_rsp_y,prc,use_AEP,GPD_TH_criterion,apply_Tides,POT_samp2,SLC,gprMdl,apply_GPD_to_SS,tide_SD,U_a,U_r,uncert_treatment)
...
if (length(ecdf_y)>=20 && Nyrs>=20) || apply_GPD_to_SS %apply GPD
    ...
    % preallocate
    HC_plt_rsp_x = NaN(size(HC_plt,1),length(HC_plt_rsp_y));
    ...
    for kk=1:size(HC_plt,1)
        ...
        % Interpolate
        HC_plt_rsp_x(kk,:) = exp(interp1(dm1,dm2,HC_plt_rsp_y','linear','extrap'));
    ...
    for kk=1:size(HC_plt,1)
        ...
        % Interpolate
        HC_plt_rsp_x(kk,:) = exp(interp1(dm1,dm2,HC_plt_rsp_y','nearest','extrap'));
...
***************  ALPHA  VERSION  **  FOR INTERNAL TESTING ONLY ************
%}
function [HC_emp,HC_plt,HC_plt_x2,HC_tbl_y,HC_plt_rsp_x,HC_tbl_rsp_x,MRL_output,str1] = StormSim_SST_Fit_SimplePar(POT_samp,Nyrs,HC_plt_x,HC_tbl_x,HC_plt_rsp_y,HC_tbl_rsp_y,prc,use_AEP,GPD_TH_criterion,apply_Tides,POT_samp2,SLC,gprMdl,apply_GPD_to_SS,tide_SD,U_a,U_r,uncert_treatment)
...
if ~strcmp(HC_plt(1).MRL_Crit,'Default') %plot these only when the GPD fit occurred
    ...
    if strcmp(MRL_output.Status,'')
        ...
        subplot(6,1,6)
        hold on
        ylim([round(min(mrl.AD_GOF_pvalue)-0.01,2) round(max(mrl.AD_GOF_pvalue)+0.01,2)]);
        yl = ylim;
        plot(mrl.Threshold,mrl.AD_GOF_pvalue,'LineWidth',2,'Color',[0.4940 0.1840 0.5560]);
        if ~isempty(TH)
            for i=1:length(TH)
                plot([TH(i) TH(i)],[yl(1) yl(2)],str{i},'LineWidth',2);
            end
...
if strcmp(MRL_output.Status,'')
    ...
    xlabel(['Threshold: ',yaxis_Label],'FontSize',12);
    ylabel({'AD GoF Test';'p-value'},'FontSize',12);
    hold off
