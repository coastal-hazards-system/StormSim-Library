function [config, structure, storm, CHS_Data, prob_mass, project_forcing, Resp] = call_ss(fout, create_plots)
...
[config, structure] = call_input_parser(fout);
t1 = toc(t1);
...
[storm, CHS_Data, prob_mass, config] = call_chs_data_formater(config);
t2 = toc(t2);
...
t6 = tic;
Resp = call_project_response(config, project_forcing, structure, prob_mass, create_plots);
t6 = toc(t6);
