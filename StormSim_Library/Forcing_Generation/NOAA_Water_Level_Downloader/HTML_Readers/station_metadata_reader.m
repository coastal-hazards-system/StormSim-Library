...
url = [exp_url station '.json?expand=details,products,datums'];
ctr2=1; % While loop trigger
while ctr2<=3
    try
        % Read URL
        metadata = webread(url,options);
        ctr2=4; % URL was read successfuly exit while loop
        ...
        if ctr2==4 % If maximum amount of tries is reached URL could not be found, throw and error
