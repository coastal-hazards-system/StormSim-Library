...
for ii = 1:length(idList)
    for pp = 1:length(prod)
        ...
        if flag4==1 && opMode ~=3
            ...
            switch opMode
                case 2
                    [indx,dummy3] = date_search(dStruct,dBeg,dEnd,indx);
                    if isempty(dummy3)==0
                        indx = indx(dummy3);
            ...
            [datum,datum_p,flag2] = datum_selector(dStruct,requested_datum);

            %% CHECK TIDAL PREDICTIONS INTERVALS
            [interval,flag3] = prediction_interval_selector(dStruct,flag1,flag2);
            ...
            [Sdata] = tidal_predictions_downloader(ctr,Sdata,datum_p,station,timezone,units,format,interval,stDatesp,endDatesp,gen_url,options,flag2,flag1,dStruct);

            %% MAKE SURE TIME VECTORS ARE THE SAME LENGTH
            switch opMode
                case 1
                    if flag2 == 0
                        [Sdata] = vector_length_check(ctr,Sdata,flag1,flag2);
                    end
                ...
                case 2
                    ...
                    Sdata(ctr).Beg = datestr(Sdata(ctr).WL.DateTime(1),'yyyy-mm-dd HH:MM');
                    Sdata(ctr).End = datestr(Sdata(ctr).WL.DateTime(end),'yyyy-mm-dd HH:MM');
                    Sdata(ii).record_length = between(Sdata(ctr).WL.DateTime(1),Sdata(ctr).WL.DateTime(end));
                    % Recompute Record Length
                    [Sdata] = record_length_calc(ctr,Sdata,flag1);
                case 3
                    Sdata(ctr).Beg = datestr(Sdata(ctr).TP.DateTime(1),'yyyy-mm-dd HH:MM');
                    Sdata(ctr).End = datestr(Sdata(ctr).TP.DateTime(end),'yyyy-mm-dd HH:MM');
                    Sdata(ii).record_length = between(Sdata(ctr).TP.DateTime(1),Sdata(ctr).TP.DateTime(end));
                    % Recompute Record Length
                    [Sdata] = record_length_calc(ctr,Sdata,flag1);
            end
...
if opMode == 3
    Sdata = rmfield(Sdata,{'WL_datum','WL_downloaded_product'});
end
%% SAVE DATA
