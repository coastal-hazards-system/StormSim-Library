...
for jj = 1:length(StationList(ii).startDate)
    if  strcmp('Predictions',StationList(ii).startDate)~=1 % if measured WL exist 
        ...
        StationList(ii).record_length = {'Predictions'};
        StationList(ii).record_length_vector = {'Predictions'};
    end
end
end