...
switch dType
    case 1 % Response Base
        if iscell(project_forcing.SWL) % Timeseries
            % Get Size Vector
            t_size = cellfun(@(x) size(x,1),project_forcing.SWL,'un',false);
            % Compute Padding Failsafe
            tpad = max(cell2mat(t_size))*2; % Ensure Tidal Signal Does Not Exceed Record Length
            if unique_rep == 1
                reps = size(project_forcing.SWL{1},2); % TO Create Tide Signal For Each replicate
            else
                reps = 1;
            end
            ...
            for ii = 1:length(t_size)
                ...
                project_forcing.SWL{ii} = project_forcing.SWL{ii} + rep_tide;
            end
        else % Peaks 
            % Get Size Vector
            t_size = size(project_forcing.SWL);
            % Create Random Tide Indexes
            tide_indx = randi([1 length(tidal_data)],t_size);
            % Add Tide To SWL
            project_forcing.SWL = project_forcing.SWL + tidal_data(tide_indx);
            for ii = 1:t_size(2)
                project_forcing.SWL(:,ii) = project_forcing.SWL(:,ii) + tidal_data(tide_indx(:,ii));
            end
        end
    case 2 % LC
