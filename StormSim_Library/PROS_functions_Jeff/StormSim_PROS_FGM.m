...
%% CONFIGURE STORMSIM: PROS SIMULATION
% Define Input CSV File Name
config.input_CSV = ['James_Island_PROS_Input_CSV.xlsx']; % Include relative path if not in parent directory
...
config = inputs_parserJAM(config);
% Added two parameters at bottom to support new uncertainty method
...
[Storm,~,config] = CHS_storm_data_prep(config);

% Prepare Storm Recurrance Rate (Only TC_Freq is Needed?) 
[TC_SRR,TC_Freq] = SRR_Extract_PROS(config.sp_ID,config.region);

% From Here you will need
% Storm
% sp_depth -> Save point Depth (Grabed from ADCIRC File)
%
...
Output variables:
    Resp (Nstm*Ndsct x 1)
        ...
        h          - Save point depth
%}
% only do following for debugging from here on5
%load workspace_before_apply_uncertainty1_17_23.mat

% Apply Uncertianty (Made changes and need to discuss)
disp('Apply uncertainty');
[Resp]=apply_forcing_uncertainty_v20230110(config,Storm,TC_Freq);
...
Output variables:
    Struc (Nstm*Ndsct x 1), unless data is a string
        ...
        Struc.km1 and km2    - Coefficients for stability equation, empty if
                               not rubble mound
%}
disp(' prep_project_structure');
[Struc] = prep_project_structure_JAM2023(config,Resp);
...
disp(' compute_structure_reponse');
[Resp] = compute_structure_response_JAM2023(Struc,Resp,config.strucType,config.storm_sampling);
...
%}
% for debugging
% load workspace_before_hazard_curves.mat
disp(' compute_hazard_curves');
tic
[OUTPUT]= compute_hazard_curves_v20230109(config,Resp);
toc
...
%save workspace_before_combineHCs_64norms.mat
